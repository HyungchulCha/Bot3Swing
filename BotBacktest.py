...
if os.path.isfile(FILE_URL_BALANCE_LIST_TEST_3M):
    os.remove(FILE_URL_BALANCE_LIST_TEST_3M)
    print('Delete!!!')
...
for x in np.nditer(xlsx_list):
    ...
    if os.path.isfile(FILE_URL_BALANCE_LIST_TEST_3M):
        obj = load_file(FILE_URL_BALANCE_LIST_TEST_3M)
        print('Loaded!!!')
    else:
        obj = {}
        save_file(FILE_URL_BALANCE_LIST_TEST_3M, obj)
        print(obj)
        print('Saved!!!')
...
prft_df.to_excel(dir + '/BacktestResult/Bot3Swing' + datetime.datetime.now().strftime('%m%d%H%M%S') + '.xlsx')